...
async function handleOFX(filePath) {
    ...
    for (const stmttrnr of stmttrnrs) {
        ...
        for (const [i, transaction] of stmttrn.entries()) {
            console.log(`:: [handleOFX] : ${path.basename(filePath)} : Importando transação ${i+1}/${stmttrn.length}`);
            await buildAndSendTransaction(transaction, acctId);
        }
    }
...
async function buildAndSendTransaction(tr, acctId) {
    ...
    const customId = `${tr.CHECKNUM}_${date}_${tr.TRNAMT}`;
